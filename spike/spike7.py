...
TIERS_PATH = (Path(__file__).parent.parent / "docs" / "data"
              / "axolotl-field-tiers.json")
...
def pinned_image(f: Findings) -> str | None:
    ...
    if not m:
        f.record("digest pin found in Dockerfile", False,
...
def ensure_image(image: str, allow_pull: bool, f: Findings) -> bool:
    ...
    if have.returncode == 0:
        ...
        return True
    if not allow_pull:
        f.record("image present locally", False, "--no-pull, and it is not here")
        return False
...
def introspect(image: str, f: Findings) -> dict | None:
    ...
    r = subprocess.run(
        ...
        input=script, capture_output=True, text=True, timeout=RUN_TIMEOUT_S,
        encoding="utf-8",
    )
    elapsed = time.perf_counter() - t0
    if r.returncode != 0:
        f.record("introspection ran", False,
...
def report(payload: dict, f: Findings) -> None:
    ...
    f.record("fields after excluding infrastructure", True,
    ...
    f.record("free-form fields (Any / dict / untyped)", True, str(c["free_form"]))

    header("CONSTRAINT EXPRESSIVENESS -- the risk this spike exists for")
    ce = payload["constraints"]
    f.record("fields carrying schema-level bounds (ge/le/gt/lt/pattern)", True,
             str(ce["with_field_constraints"]))
    f.record("fields typed as an enum or Literal", True, str(ce["enum_typed"]))
    f.note("model_validator hooks (cross-field rules in RUNTIME code)",
    ...
    for name, found in payload["correctness_settings"].items():
        f.record(f"correctness setting: {name}", bool(found),
                 found if isinstance(found, str) else
                 ", ".join(found) if found else "NOT FOUND under any tried name")
...
def decide(payload: dict, f: Findings) -> dict:
    ...
    if mostly_typed and constraints_visible:
        decision = "DERIVE"
        reason = ("the schema is typed and carries most of its own "
                  "constraints, so a generated form refuses what the trainer "
                  "refuses")
    elif mostly_typed:
        decision = "DERIVE THE FORM, HAND-WRITE THE RULES"
        reason = ("types and defaults are generable, but the cross-field "
        ...
        reason = ("too much of the schema is free-form to generate a usable "
                  "form from")
    ...
    out = {
        ...
        "surface_size_verdict":
            "a UI" if c["training_relevant"] <= 80 else
            "large but tractable" if c["training_relevant"] <= 200 else
            "a different product -- tiering is mandatory, not optional",
    }
    print(f"  {decision}\n  {reason}")
    f.record("advanced mode is derivable from the schema",
             decision.startswith("DERIVE"), reason)
    return out
...
def check_tiers(payload: dict, f: Findings) -> dict:
    ...
    if not TIERS_PATH.exists():
        f.record("tier classification exists", False,
    ...
    f.record("every classified field exists in the pinned schema", not unknown,
             "all present" if not unknown
    ...
    f.note("share of the sample that is KNOWN BUT UNSUPPORTED",
