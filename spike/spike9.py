...
TOKENIZER_URL = f"https://huggingface.co/{TOKENIZER_REPO}/resolve/main/tokenizer.json"
...
_SUBJECTS = ["the invoice", "our refund policy", "the API key rotation",
             "the shipping delay", "the seat upgrade", "the failed webhook",
             "the duplicate charge", "the expired certificate"]
_OPENERS = ["I need help with", "Can you explain", "What happens to",
            "Could someone look at", "I'm confused about"]
_BODY = (
...
def generate(path: Path, target_bytes: int, f: Findings) -> dict:
    ...
    if path.exists() and path.stat().st_size >= target_bytes * 0.98:
        f.note("dataset reused", f"{path.name} already {path.stat().st_size / 1e9:.2f} GB")
        return {"path": str(path), "bytes": path.stat().st_size, "generated": False}
    ...
    with path.open("wb") as fh:
        ...
        while written < target_bytes:
            line = (json.dumps(synth_row(rng, rows), ensure_ascii=False) + "\n").encode()
            buf += line
            written += len(line)
            rows += 1
            if len(buf) >= 8 << 20:
    ...
    f.record("dataset generated", True,
             f"{path.name}: {written / 1e9:.2f} GB, {rows:,} rows, "
             f"{elapsed:.1f}s ({written / 1e6 / elapsed:.0f} MB/s write)")
    return {"path": str(path), "bytes": written, "rows": rows,
            "generated": True, "write_seconds": round(elapsed, 2)}
...
class Measurement:
    ...
    def mb_per_s(self) -> float:
        return self.bytes_processed / 1e6 / self.seconds if self.seconds else 0.0
    ...
    def to_dict(self) -> dict:
        return {
            ...
            if self.bytes_processed else None,
            **self.detail,
        }
...
def measure(label: str, fn, size_bytes: int, f: Findings) -> Measurement:
    with RSSSampler() as s:
        ...
        detail = fn()
        elapsed = time.perf_counter() - t0
    m = Measurement(label, elapsed, size_bytes, s.peak, s.baseline, detail or {})
    f.record(label, True,
...
def load_tokenizer(f: Findings):
    ...
    if not local.exists():
        try:
            ...
            f.note("tokenisation cost is NOT MEASURED",
                   "fell back to str.split(); every tokenisation number in this "
                   "run is a placeholder and must not be quoted")
            return (lambda text: len(text.split())), "FALLBACK str.split (NOT a tokeniser)"
    try:
    ...
    f.record("tokenizer loaded", True,
...
def run_size(size_gb: float, counter, f: Findings) -> dict:  # noqa: C901
    ...
    def summarise(rep) -> dict:
        return {"row_count": rep.row_count, "usable_rows": rep.usable_rows,
                "valid": rep.valid, "errors": rep.error_count,
    ...
    validate_only = measure(
        ...
        lambda: summarise(stream_validate(path)), size, f)
    ...
    no_lines = measure(
        ...
        lambda: summarise(stream_validate(path, track_lines=False)), size, f)
    ...
    tokenised = measure(
        ...
        lambda: summarise(stream_validate(path, count_tokens=counter,
                                          byte_limit=limit)),
        tok_bytes, f)
    ...
    token_share = (1 - validate_seconds_for_tok_bytes / tokenised.seconds
                   if tokenised.seconds else 0)
    result["tokenisation"] = {
        ...
        "slowdown_multiple": round(
            tokenised.seconds / validate_seconds_for_tok_bytes, 2)
        if validate_seconds_for_tok_bytes else None,
        "whole_file_seconds": {
            "value": round(size / 1e6 / tokenised.mb_per_s, 1)
            if tokenised.mb_per_s else None,
            "kind": "measured" if limit is None else "derived from the prefix rate",
        },
    }
    result["line_number_guarantee"] = {
        ...
        "share_of_validate_time": round(
            (validate_only.seconds - no_lines.seconds) / validate_only.seconds, 4)
        if validate_only.seconds else None,
        "extra_rss_mb": round(
            (validate_only.rss_growth_bytes - no_lines.rss_growth_bytes) / 1e6, 1),
        "caveat":
            "One run each, back to back, on a machine doing other things. A "
    ...
    print(f"  -> tokenisation is {token_share * 100:.0f}% of the tokenising pass")
    print(f"  -> line numbers cost "
    ...
    if size_gb <= IN_MEMORY_CEILING_GB:
        from api.validation import validate as in_memory_validate
        in_mem = measure(
            "in-memory (shipped): parse + validate",
            lambda: {"usable_rows": in_memory_validate(path).usable_rows}, size, f)
        ...
        result["memory_multiplier"] = {
            ...
            "streaming_x_file": round(validate_only.rss_growth_bytes / size, 4),
            "adr_0005_claim": 4.8,
        }
    else:
        f.note("in-memory validator skipped", f"{size_gb} GB would need roughly "
...
def derive_ceiling(results: list[dict], f: Findings) -> dict:
    ...
    tok_rates = [r["passes"]["validate_and_tokenise"]["mb_per_s"] for r in results]
    ...
    flat = {
        ...
        "file_size_ratio": round(max(sizes) / min(sizes), 1) if len(sizes) > 1 else 1.0,
        "rss_growth_ratio": round(max(growth) / min(growth), 2)
        if len(growth) > 1 and min(growth) > 0 else None,
    }
    is_flat = (flat["rss_growth_ratio"] is None
               or flat["rss_growth_ratio"] < 2.0)
    f.record("peak memory is flat as the file grows", is_flat,
    ...
    verdict = {
        ...
        "tolerable_wait_justification":
            "the point at which a synchronous upload page stops reading as a "
            "wait and starts reading as a hang. A judgement, not a measurement.",
        "gb_validatable_within_tolerable_wait":
            round(mean_rate * TOLERABLE_S / 1000, 1),
        "gb_validatable_and_tokenisable_within_tolerable_wait":
            round(mean_tok_rate * TOLERABLE_S / 1000, 1),
        "current_ceiling_gb": 1.0,
        "current_ceiling_reason":
            "in-memory validation at ~4.8x the file size (ADR-0005)",
    }
    ...
    verdict["recommendation"] = (
        ...
        if splits else
        "ONE PASS. Validation and token counting both fit inside a tolerable "
    ...
    verdict["sensitivity"] = {
        ...
        "five_gb_tokenise_seconds_if_rate_were": {
            f"x{m}": round(5000 / (mean_tok_rate * m))
            for m in (1, 2, 4, 10)
        },
        ...
        "conclusion":
            "Tokenisation would have to be an order of magnitude faster than "
    ...
    f.record("token counting fits in one synchronous pass", not splits,
...
def main() -> int:
    ...
    ap.add_argument("--sizes", default=",".join(str(s) for s in DEFAULT_SIZES_GB),
    ...
    print(f"  sizes: {sizes} GB -- needs {needed / 1e9:.0f} GB, "
          f"{free / 1e9:.0f} GB free")
    if needed > free * 0.8:
        f.record("disk space", False,
