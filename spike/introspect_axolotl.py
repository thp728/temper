...
def is_infrastructure(name: str) -> bool:
    return name in INFRASTRUCTURE_EXACT or name.startswith(
        INFRASTRUCTURE_PREFIXES
    )
...
def is_free_form(annotation) -> bool:
    ...
    return bool(
        ("dict" in text.lower() and "Any" in text)
...
def collect() -> dict:
    ...
    if model is None:
        return {
            "error": f"no config model importable. Last: {import_error}",
    ...
    total_constraints = (
        expressed + len(model_validators) + len(field_validators)
    )
    ...
    return {
        ...
        "constraints": {
            ...
            "expressed_in_schema_fraction": (
                round(expressed / total_constraints, 3)
