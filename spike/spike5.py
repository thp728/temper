...
def read_storage_bounds(f: Findings) -> dict:
    ...
    client_side_bound = "storage" in source and any(
        tok in source
        for tok in ("_validate_storage", "storage >", "storage <")
    )
    ...
    f.note(
        "where the storage ceiling lives",
        ...
        if not client_side_bound
        else "the SDK bounds it locally",
    )

    return {
        "default_storage_gb": default,
        "client_side_upper_bound": client_side_bound,
        "ceiling_is": "documented"
        if client_side_bound
        else "discovered by rejection",
    ...
    for size in STORAGE_LADDER_GB:
        ...
        try:
            instance = client.instances.create(
                ...
                template="vm",
                storage=size,
                name=f"{INSTANCE_NAME}-{size}",
            )
        except Exception as e:  # noqa: BLE001 - a rejection is the measurement
            message = f"{type(e).__name__}: {e}"
            attempts.append(
                {"storage_gb": size, "accepted": False, "error": message}
            )
            ...
            if bound is not None and named_ceiling is None:
                named_ceiling = bound
                f.record(
                    "the API NAMES its storage ceiling",
                    True,
...
def price_the_storage(
    client: Client, gpu: str, size_gb: int, baseline: dict, f: Findings
) -> dict:
    ...
    f.record(
        "the GPU-hour rate is independent of disk size",
        unchanged,
    ...
    f.note(
        ...
        if unchanged
        else "the GPU rate moved with the disk, so storage is inside it and the "
...
def run_bootstrap5(
    ssh_command: str, repo: str, fill_gb: int, f: Findings
) -> dict | None:
    header("ON-VM PROBE")
...
def interpret(report: dict, requested_gb: int, f: Findings) -> dict:
    ...
    f.record(
        "the disk the API accepted is the disk that exists",
        honoured,
    ...
    f.record(
        ...
        bool(report["write_test_ok"]),
        report.get("write_test_rate")
        or report.get("write_test_error", "")[:200],
    )
    ...
    if report.get("download_ok"):
        ...
        dl = {
            ...
            "hf_transfer_note": "hf_transfer was deliberately NOT enabled. The measured rate is "
        ...
        f.record(
            "download measured", False, report.get("download_error", "")[:400]
        )
    ...
    verdict = {
        ...
        "statement": (
            ...
            if supports_70b
            else "The disk clears the 200 GB kill line but is under the 250-400 GB "
            ...
            "does not."
            if above_kill_line
            else "DISK CANNOT EXCEED ~200 GB. 70B FULL FINE-TUNING IS OFF THE TABLE "
...
def main() -> int:
    ...
    storage_gb = None
    try:
        instance, storage_gb, attempts = create_with_largest_disk(
            client, gpu, f
        )
        ...
        payload["pricing"] = price_the_storage(
            client, gpu, storage_gb, pricing, f
        )
