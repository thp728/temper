...
from api import catalog, config, db, feasibility, orchestrator
...
def create(dataset_id: str, base_model: str, hyperparameters: dict) -> str:
    ...
    warn = feasibility.warning(feasibility.usable_rows(ds),
                               hyperparameters, config.MAX_JOB_DURATION_S)
    warnings = [warn] if warn else []

    job_id = db.create_job(dataset_id, base_model, hyperparameters,
                           warnings=warnings)
    if warn:
