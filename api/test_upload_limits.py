...
def upload(client, data, name="d.jsonl"):
    return client.post("/v1/datasets",
...
def test_default_limit_is_one_gigabyte(monkeypatch):
    ...
    assert config._megabytes("TEMPER_MAX_DATASET_MB", 1024) * 1024 * 1024 \
        == 1024 ** 3
...
def test_dataset_over_limit_refused_with_code_and_both_sizes(client, monkeypatch):
...
def test_refusal_names_the_removal_path(client, monkeypatch):
    ...
    monkeypatch.setattr(config, "MAX_DATASET_BYTES", 64 * 1024)
    body = upload(client, jsonl_bytes(chat_rows(400, pad=300))).json()["detail"]
    ...
    * validation on the loop (the defect): the loop itself is frozen inside
    ...
    async def scenario():
        ...
        async with AsyncClient(transport=transport, base_url="http://test") as c:
            ...
            status = (await upload_task).status_code
