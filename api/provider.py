...
class JarvisLabsProvider:
    ...
    def select_gpu(self, preference: Sequence[str]) -> GpuChoice:
        avail = {r.gpu_type: r for r in self._client.account.gpu_availability()
        ...
        return GpuChoice(gpu, avail[gpu].price_per_hour,
                         self._client.account.currency())
    ...
    def create(self, gpu_type: str, storage_gb: int, name: str) -> Machine:
        ...
        created = self._client.instances.create(
            gpu_type=gpu_type, num_gpus=1, template="vm",
            storage=storage_gb, name=name)
    ...
    def await_ready(self, machine: Machine) -> str:
        ...
        while time.time() - t0 < SSH_READY_TIMEOUT_S:
            try:
                r = subprocess.run(_ssh(machine.handle) + ["true"],
    ...
    def push(self, machine: Machine, payload: bytes, dest: str) -> None:
        ...
        r = subprocess.run(
            ...
            input=payload, capture_output=True, timeout=PUSH_TIMEOUT_S)
        if r.returncode != 0:
            raise OrchestratorError(
    ...
    def fetch(self, machine: Machine, path: str) -> bytes:
        r = subprocess.run(_ssh(machine.handle) + [f"sudo cat {path}"],
