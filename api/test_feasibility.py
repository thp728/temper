...
from api import config, feasibility  # noqa: E402
...
def test_no_estimate_when_max_steps_caps_the_run():
    ...
    assert feasibility.estimated_duration_s(
        usable_rows=10 ** 9, hyperparams={"max_steps": 10}) is None
...
def test_warning_fires_when_estimate_exceeds_the_ceiling():
    w = feasibility.warning(usable_rows=10 ** 6, hyperparams={},
...
def test_no_warning_when_the_run_plausibly_fits():
    assert feasibility.warning(usable_rows=500, hyperparams={},
                               max_duration_s=config.MAX_JOB_DURATION_S) is None
...
def test_warning_at_exact_equality_does_not_fire():
    ...
    rows = int(config.MAX_JOB_DURATION_S * feasibility.ROWS_PER_SECOND
               / feasibility.DEFAULT_EPOCHS)
    assert feasibility.warning(usable_rows=rows, hyperparams={},
                               max_duration_s=config.MAX_JOB_DURATION_S) is None


def test_default_epochs_is_pinned_to_the_trainer_default():
    """Duplicated from trainer/entrypoint.py on purpose -- the control plane
    does not import trainer code -- but the duplication is pinned here, so a
    trainer default change fails this test instead of silently skewing every
    estimate."""
...
def test_large_dataset_yields_a_warning_and_a_launched_job(client, monkeypatch):
...
def test_warning_is_on_the_job_afterwards_and_in_its_history(client, monkeypatch):
