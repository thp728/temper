...
from thinking import MixedThinkingDataset, detect as detect_thinking  # noqa: E402
...
def validate(path: Path, messages_field: str = "messages") -> Report:
    ...
    try:
        ...
        rep.errors.append(Issue(None, "encoding",
    ...
    for line_no, line in enumerate(text.splitlines(), start=1):
        ...
        try:
            ...
            rep.errors.append(Issue(line_no, "invalid_json",
                                    f"Not valid JSON: {e.msg} at column {e.colno}."))
            continue
        if not isinstance(obj, dict):
            rep.errors.append(Issue(line_no, "not_an_object",
    ...
    if not with_msgs:
        keys = sorted({k for r in rows[:20] for k in r if not k.startswith("_")})
    ...
    rep.preview = [{k: v for k, v in r.items() if k != "_line"}
